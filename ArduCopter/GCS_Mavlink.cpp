...
void GCS_MAVLINK_Copter::handle_rc_channels_override(const mavlink_message_t &msg)  // is this right? I have no idea
{
    ...
    //uint16_t l = 0;                         // create variabele l and make l zero
    uint16_t datach5 = 1100;

    //if (k <= 1494) {        // if ch4 (=YAW) isn't centered, make l one
    //    l = 1;
    //}
    //if (k >= 1534) {
    //    l = 1;
    ...
    if (override_data[4] <= 1150) {
        datach5 = 988;
    }
    if (override_data[4] >= 1151) {
        if (override_data[4] <= 1250) {
            datach5 = 1500;
        }
    }
    if (override_data[4] >= 1251) {
        datach5 = 2012;
    }

    if (i >= 1850) {                                               // if ch6 from the pilot is high then this part will work
        RC_Channels::set_override(8, override_data[0], 100);      // ch9 from the pilot will be overrided by ch1 from MavLink
        RC_Channels::set_override(9, override_data[1], 100);      // ch10 from the pilot will be overrided by ch2 from MavLink
        RC_Channels::set_override(10, datach5, 100);     // ch11 from the pilot will be overrided by ch5 from MavLink
        RC_Channels::set_override(11, override_data[2], 100);     // ch12 from the pilot will be overrided by ch3 from MavLink
        RC_Channels::set_override(12, override_data[5], 100);     // ch13 from the pilot will be overrided by ch6 from MavLink
        if (k >= 1494) {                             // if ch4 (=YAW) from the pilot is centered then this part will work
            if (k <= 1534) {
                //l = 0;
                RC_Channels::set_override(3, override_data[3], 20);   // ch4 (=YAW) from the pilot will be override by ch4 from MavLink
            }
        }
        //if (l == 1) {
        ...
        RC_Channels::set_override(11, 1500, 100);                  // ch 12 from the pilot will be overrided by 1500
        RC_Channels::set_override(12, 1950, 100);                  // ch 13 from the pilot will be overrided by 1500
    }
